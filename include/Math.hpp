...
/*
This file (Math.hpp) is separate from the rest of the plugin codebase and is licensed under whatever license UEVR (https://github.com/praydog/UEVR) project settles on
...
static glm::mat4 remove_y_component(const glm::mat4 &mat)
{
	...
	return glm::rowMajor4(
		glm::lookAtLH(Vector3f{}, Vector3f{forward_dir}, Vector3f(0.0f, 1.0f, 0.0f)));
}

static quat to_quat(const vec3 &v)
{
	const auto mat =
		glm::rowMajor4(glm::lookAtLH(Vector3f{0.0f, 0.0f, 0.0f}, v, Vector3f{0.0f, 1.0f, 0.0f}));
